...
def get_all_chapters(
    ...
    return [
        ...
        for book_index in range(len(book_ids))
        for chapter_index in range(book_chapters[book_ids[book_index]])
    ]
...
def scrape_verses_by_chapter(chapter: Chapter) -> list[Verse]:
...
with ThreadPoolExecutor() as executor:
    results = list(executor.map(scrape_verses_by_chapter, all_chapters))
